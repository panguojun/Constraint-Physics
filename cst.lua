...
function nc_1(vc,cst,cd)
	...
	if(length(pw - cd.o) > 0.5) then
		...
		cd.o = pw
		--	arrow(pw.x,pw.y,(pw + cd.ux*20).x,(pw +cd.ux*20).y)
		--	arrow(pw.x,pw.y,(pw + cd.uy*20).x,(pw +cd.uy*20).y)
		return true
	end
	return false
end
...
link_cst_r.place = function(label,Vw,cst,cd)
		...
		dumpv(label .. " place------------------", Vc)
		psz(6);pix(cd.o.x, cd.o.y);text2(cd.o.x, -28+cd.o.y, label);psz(1)
